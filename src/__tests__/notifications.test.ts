...
describe("notification message builders", () => {
  ...
  describe("cortex_init", () => {
    it("successMsg returns fixed message", () => {
      const result = TOOL_NOTIFICATIONS.cortex_init.successMsg({}, "✓ Created");
      expect(result).toBe(".cortex directory created");
    });
  });

  describe("cortex_configure", () => {
    it("successMsg extracts model name from full path", () => {
      const args = { primaryModel: "anthropic/claude-sonnet-4-20250514" };
      const result = TOOL_NOTIFICATIONS.cortex_configure.successMsg(args, "✓ Configured");
  ...
  describe("common errorMsg behavior", () => {
    it("truncates to 100 chars for tools with simple errorMsg", () => {
      ...
      const tools = ["plan_save", "plan_delete", "session_save", "docs_save", "docs_init", "cortex_init", "cortex_configure"];
