...
describe("MCP Server", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "cortex-mcp-"));
  ...
  describe("cortex_init tool", () => {
    it("should initialize .cortex directory structure", async () => {
      const cortexPath = path.join(tmpDir, ".cortex");
      expect(fs.existsSync(cortexPath)).toBe(false);
      ...
      expect(fs.existsSync(path.join(cortexPath, "plans"))).toBe(true);
      expect(fs.existsSync(path.join(cortexPath, "sessions"))).toBe(true);
    });

    it("should handle existing .cortex directory", () => {
      const cortexPath = path.join(tmpDir, ".cortex");
  ...
  describe("cortex_status tool", () => {
    it("should report missing .cortex directory", () => {
      const cortexPath = path.join(tmpDir, ".cortex");
      expect(fs.existsSync(cortexPath)).toBe(false);
    });
  ...
  describe("plan_save tool", () => {
    it("should save plan with title, type, and content", () => {
      const plansPath = path.join(tmpDir, ".cortex", "plans");
      fs.mkdirSync(plansPath, { recursive: true });
...
\`\`\``;
  ...
  describe("session_save tool", () => {
    it("should save session with summary and decisions", () => {
      const sessionsPath = path.join(tmpDir, ".cortex", "sessions");
      fs.mkdirSync(sessionsPath, { recursive: true });
  ...
  describe("cortex_configure tool", () => {
  ...
  describe("Input Schema Validation", () => {
    it("should define input schema for all tools", () => {
      const schemas = {
        cortex_init: { type: "object", properties: {} },
        cortex_status: { type: "object", properties: {} },
        cortex_configure: {
          type: "object",
          properties: {
    ...
    it("should have required fields for parameterized tools", () => {
      const requiredFields = {
        cortex_configure: ["scope", "primaryModel", "subagentModel"],
        worktree_open: ["name"],
