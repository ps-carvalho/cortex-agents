...
import { CortexEngine } from "../index.js";
...
describe("Database & Schema", () => {
  ...
  it("getDefaultDbPath returns a path under ~/.config/cortex-agents", () => {
    const p = getDefaultDbPath();
    expect(p).toContain(path.join(".config", "cortex-agents", "cortex.db"));
  });
...
describe("TargetStore", () => {
  ...
  it("AgentTargetConfig with JSON fields round-trips correctly", () => {
    ...
    const config: AgentTargetConfig = {
      ...
      target_id: "claude",
      native_name: null,
      tools_override: ["Read", "Glob", "Grep", "mcp__cortex-agents__plan_save"],
    ...
    expect(result!.extra_frontmatter).toEqual({ mcpServers: "cortex-agents" });
...
describe("Claude Renderer", () => {
  ...
  it("architect rendered output has tools with MCP-prefixed cortex tools", () => {
    const renderer = getRenderer("claude", db)!;
    const output = renderer.renderAgent("architect");
    ...
    // MCP tools
    expect(output).toContain("mcp__cortex-agents__plan_save");
  });
...
describe("CortexEngine", () => {
  ...
  it("initialize seeds on first run and returns SeedResult", () => {
    engine = new CortexEngine(dbPath);
