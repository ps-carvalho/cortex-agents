...
function makeTmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "cortex-repl-"));
}
...
describe("state management", () => {
  ...
  it("returns null for corrupted state file", () => {
    fs.writeFileSync(path.join(tmpDir, ".cortex", "repl-state.json"), "not json");
    expect(readReplState(tmpDir)).toBeNull();
  });

  it("creates .cortex directory if missing", () => {
