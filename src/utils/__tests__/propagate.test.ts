...
describe("propagatePlan", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "cortex-prop-"));
  ...
  it("copies all plans when no specific filename given", () => {
    const sourcePlans = path.join(sourceDir, ".cortex", "plans");
    ...
    const targetPlans = path.join(targetDir, ".cortex", "plans");
  ...
  it("copies config.json if it exists", () => {
    const sourceCortex = path.join(sourceDir, ".cortex");
    const sourcePlans = path.join(sourceCortex, "plans");
    fs.mkdirSync(sourcePlans, { recursive: true });
    fs.writeFileSync(path.join(sourcePlans, "plan.md"), "Plan");
    fs.writeFileSync(path.join(sourceCortex, "config.json"), '{"key": "value"}');
    ...
    const targetConfig = path.join(targetDir, ".cortex", "config.json");
  ...
  it("creates sessions dir in target", () => {
    ...
    const targetSessions = path.join(targetDir, ".cortex", "sessions");
